...
public final class LocationQuery {
	...
	private final String columnNames = SQLColumnNamesUtil.join(SQLColumnNamesUtil.LOCATION, "location");
	...
	public final String EXISTS_BY_ID = "SELECT EXISTS (SELECT 1 FROM location WHERE location_id = :locationID);";
	...
	public final String SELECT_BY_NAME = String.format("SELECT %s FROM location WHERE name = :name;", columnNames);
